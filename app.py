...
class Calculator:
    ...
    def __dhw_input(self, demand_old):
        number = st.text_input('1. Justere varmtvannsbehovet [kWh/år]?', value = demand_old, help = "Varmtvannsforbruket itgjør ca. 15% av det årlige strømforbruket, men dette er avhengig av hvor mange personer det er i husholdningen.")
        if number.isdigit():
            number = float(number)
            if number < 0:
    ...
    def sizing_results(self):
        with st.container():
            ...
            with column_1:
                ...
                if self.number_of_boreholes > 1:
                    self.__render_svg_metric(svg, "Brønndybde", f"{self.number_of_boreholes} {well_description_text} á {self.__rounding_to_int(self.borehole_depth)} m<br><small>(forutsatt 15 m avstand mellom brønnene)</small>")
                else:
            ...
            with column_2:
                ...
                self.__render_svg_metric(svg, "Varmepumpestørrelse", f"{self.heat_pump_size} kW<br><small>(+ {self.__rounding_to_int(np.max(self.peak_series))} kW spisslast)</small>")
